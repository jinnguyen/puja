...
class PujaCompiler
{
    ...
    public $_custom_filter;
    public $_custom_tags;
    public $_filter;
    public $_tags;
    public $_core_matches;
    public $_operators = array(' and ', ' or ', ' not ', ' in ', ' is ', '%', '!==', '!=', '>=', '<=', '===', '==', '<>', '>', '<', '&&', '||', '!', '+', '-', '*', '/', '=', ';', '__seperate__', '__array_split__');
    public $_cache;
    public $_include_content = array();
    public $_mtime = 0;
    public $_data = array();
     ...
     * @return string|null
     */
    public function get_template_content_callback($matches)
    {
     ...
     */
    public function get_template_content($tpl_file)
    {
     ...
     * @return string
     */
    public function get_block_extends($content)
    {
        preg_match('/\{\%\s*extends\s?(.*?)\s?\%\}/is', $content, $matches);
        if (count($matches) == 0) {
            return $this->remove_remain_block($content);
        }

        $extends_content = $this->get_template_content($matches[1]);
        // TODO ugly, maybe use named regex group?
    ...
    public function remove_remain_block($content)
    {
     ...
     */
    public function compile_variable($var, $var_prefix = '$', $default_value = 'null')
    {
        ...
        if ($var === '' || $var === null || is_numeric($var)) {
            return $var;
        }
     ...
     * @internal param string $check_isset if $check_isset is set, it will add
     * isset() into template* if $check_isset is set, it will add isset() into template
     * @return mixed|string
     */
    public function compile_variable_filter($var)
    {
        $var = trim($var);
        if (!$var) {
            return $var;
     ...
     * @return string
     */
    public function compile_start($content)
    {
     ...
     * @return string
     */
    public function compile_end($content)
    {
        return str_replace(array('[:lpuja_variable:]', '[:lpuja_percent:]', '[:lpuja_specialvar:]', '[:lpuja_comment:]'), array('{{', '{%', '{$', '{#'), $content);
    }
     ...
     * @return string
     */
    public function get_block_include($content)
    {
     ...
     */
    public function remove_template_comment($content)
    {
     ...
     * @return string
     */
    public function compile_in_array($var)
    {
     ...
     */
    public function remove_quote($string)
    {
     ...
     * @return string
     */
    public function build_query($formdata, $numeric_prefix = null, $arg_separtor = '&')
    {
     ...
     * @return string
     */
    public function compile_include_variable($var_str, $type_include)
    {
    ...
    {
        // TODO method is very big, need complex refactoring
        // TODO feature: split to 2 methods: parseFile, parseString
            ...
            if (count($matches[1])) foreach ($matches[1] as $key => $tag) {
                switch ($tag) {
                    ...
                    case 'empty':
                        ...
                        break;
                    case 'print':
                        $matches_replace[$key] = '\';print_r(' . $matches[2][$key] . '); $ast_puja_template .= \'';
                        break;
                    case 'set':
                    case 'before_include':
                    case 'after_include':
